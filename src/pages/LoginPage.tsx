    ...
    <main className="flex flex-col justify-center items-center px-16 py-20 font-extrabold text-black bg-white max-md:px-5 h-screen">
      <div className="flex flex-col mt-32 w-full max-w-[1111px] max-md:mt-10 max-md:max-w-full">
        ...
        <form onSubmit={handleSubmit} className="w-full">
          <InputField label="아이디" placeholder="아이디를 입력해주세요." type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
          <InputField label="비밀번호" placeholder="비밀번호를 입력해주세요." type="password" value={password} onChange={(e) => setPassword(e.target.value)} />

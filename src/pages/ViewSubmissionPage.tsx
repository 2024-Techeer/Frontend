...
const ViewSubmissionPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchSubmissionData = async () => {
      try {
        ...
        // 신청서 양식 가져오기
        const applicationResponse = await axios.get(`http://localhost/api/v1/recruitments/${recruitmentId}/applications`, { headers });
        setApplication(applicationResponse.data);

        // 제출된 답변 가져오기
        const submissionResponse = await axios.get<SubmissionData>(`http://localhost/api/v1/applications/${applicationResponse.data.applicationId}/submissions/${submissionId}`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
  ...
  const handleAccept = async () => {
    try {
      ...
      await axios.patch(`http://localhost/api/v1/applications/${application.applicationId}/submissions/${submissionId}/accepting`, {}, { headers });
      alert('신청서 수락 완료!');
  ...
  const handleReject = async () => {
    try {
      ...
      await axios.patch(`http://localhost/api/v1/applications/${application.applicationId}/submissions/${submissionId}/rejecting`, {}, { headers });
      alert('신청서 거절 완료!');
      navigate(`/Recruitment_Owner/${recruitmentId}`);
    } catch (err) {
